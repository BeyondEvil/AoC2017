...
for line in DATA:
    ...
    if not re.search(hasvowels, line):
        isnice1 = False
    if not re.search(hasapair, line):
        isnice1 = False
    if re.search(hasgroups, line):
        isnice1 = False

    if not re.search(hasdpair, line):
        isnice2 = False
    if not re.search(haspairsep, line):
        isnice2 = False

...
class Node(object):
    ...
    def children_repr(self):
        _children = []
        for child in self.children:
            _children.append(child.name)
        return _children

    def add_children_weights(self):
        for child in self.children:
            self.total_weight += child.weight

    def build_child_weights(self):
        for child in self.children:
            self.children_weights.append(child.total_weight)

    def is_unbalanced(self):
        # are my children unbalanced?
        if self.children_weights:
            highest = max(self.children_weights)
            lowest = min(self.children_weights)
            return highest != lowest
        return False

    def get_unbalanced_child(self):
        w = max(self.children_weights)
        m = min(self.children_weights)
        for child in self.children:
            if child.total_weight == w:
                return w - m

    def check_sibling_weights(self):
        sibling_weights = set()
        print("sibling weights: ", sibling_weights)
        if self.parent:
            for sibling in self.parent.children:
                print("sibling name: ", sibling.name)
                print("sibling total weight: ", sibling.total_weight)
                sibling_weights.add(sibling.total_weight)
                print("sibling weights: ", sibling_weights)
            return len(sibling_weights)

    def check_children_weights(self):
        return len(set(self.children_weights))


def run_it_a(programs):
    match = "\(|\)"
    nodes = set()
    parents = {}
    rows = programs.split('\n')
    ...
    #     print("Is leaf: ", node.is_leaf())
    #     print("")
    #     #if node.is_parent and not node.parent:
    #         #print("ROOT of tree is: ", node.name)

    return nodes


def run_it_b(programs):
    nodes = run_it_a(programs)

    # for node in nodes:
    #     node.add_children_weights()
    #
    # for node in nodes:
    #     node.build_child_weights()

    # for node in nodes:
    #     print("name: ", node.name)
    #     print("unbalanced: ", node.is_unbalanced())
    #     print("weight: ", node.weight)
    #     print("total weight: ", node.total_weight)
    #     print("children: ", node.children_repr())
    #     print("childrens weights: ", node.children_weights)
    #     print("")
    ...
    print(root)
    exit(0)
    unbalanced_nodes = set()
    for node in nodes:
        if node.is_unbalanced():
            unbalanced_nodes.add(node)

    for node in unbalanced_nodes:
        print node.check_children_weights()
    exit(0)
    for node in unbalanced_nodes:
        print node.check_sibling_weights()

    print(unbalanced_nodes)
    root = find_root(unbalanced_nodes)
    print("root: ", root)
    ...
    exit(0)
    latest_unbalanced_node = []

    def find_unbalanced(_root):
        if not _root.children:
            return
        if _root.is_unbalanced():
            latest_unbalanced_node.append(_root.get_unbalanced_child())
        for _child in _root.children:
            find_unbalanced(_child)
        return

    latest_unbalanced_node = []
    root = find_root()
    find_unbalanced(root)
    print(latest_unbalanced_node)
    return 0
...
cntj (57)"""
    ...
    _programs = """mypyemv (1058) -> tdssotr, pebnvks, zaulju
snslv (48)
aytbgf (30)
mkcrsy (42)
shwcd (42)
...
tnmtz (26)"""
    #print run_it_a(_programs)

    print run_it_b(_programs)

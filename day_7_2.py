...
class Node(object):
    ...
    def __str__(self, level=0):
        ret = "\t"*level+repr(self.name)+repr(self.get_total_weight())+"\n"
...
def run_it_a(programs):
    ...
    for child in node_parent.children:
        weights.append(child.get_total_weight())

    diff = max(weights) - min(weights)
    print(root.name + " should have weight: ", root.weight - diff)

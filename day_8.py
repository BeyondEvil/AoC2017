...
def run_it_a(datainput):
    rows = datainput.split('\n')
    ...
    print("largest value: ", max(registry.values()))


def run_it_b(datainput):
    rows = datainput.split('\n')
    ...
    for row in rows:
        ...
        if operators[operator](registry[conditional_reg], int(cond_value)):
            registry[register_to_modify] = operators[operation](registry[register_to_modify], int(op_value))
        if registry[register_to_modify] >= largest_held:
            largest_held = registry[register_to_modify]

    print("largest held: ", largest_held)


if __name__ == '__main__':

    _input2 = """b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10"""

    _input = """y inc 497 if n <= 3
ig inc -54 if es < 9
j dec 278 if low < 10
nm inc -531 if tr == 0
tq inc 537 if tq < 9
...
# p = p.replace('inc', '+').replace('dec', '-')
# instructions = [l for l in p.splitlines() if l]
# registers = set(i.split()[0] for i in instructions)
# record = 0
#
# for r in registers:
#     exec(f'{r} = 0')
...
#     exec(i)
#     if eval(r) > record:
#         print(f'new record: {eval(r)}')
#         record = eval(r)
#
# values = ({r: eval(r) for r in registers})
